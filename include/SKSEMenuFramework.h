...
// PATCHED for HUDWidgets-Claude (2026-04-28):
// Original line was `static auto menuFramework = GetModuleHandle(...)` which
// resolves at DllMain time. SKSE loads plugin DLLs alphabetically; if our DLL
// loads before SKSEMenuFramework.dll, menuFramework is null forever and every
// GetProcAddress silently fails. Switch to a lazy resolver with LoadLibrary
// fallback so first use happens after all plugins are loaded.
namespace SKSEMenuFrameworkHandle_Internal_ {
    inline HMODULE Resolve() {
        HMODULE m = ::GetModuleHandleW(L"SKSEMenuFramework");
        if (!m) {
            m = ::LoadLibraryW(L"Data/SKSE/Plugins/SKSEMenuFramework.dll");
        }
        return m;
    }
    inline HMODULE Get() {
        static HMODULE h = Resolve();
        return h;
    }
}
#define menuFramework (::SKSEMenuFrameworkHandle_Internal_::Get())
